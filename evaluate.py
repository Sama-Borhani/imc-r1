...
def plot_mid_and_spread(df: pd.DataFrame, product: str) -> None:
    ...
    plt.plot(sub[TIMESTAMP_COL], sub["mid"])
    ...
    plt.figure(figsize=(12, 5))
    plt.plot(sub[TIMESTAMP_COL], sub["spread"])
...
def main() -> None:
    ...
    with open(SUMMARY_DIR / "round1_summary.json", "w") as f:
        json.dump(summary, f, indent=2)

    print(json.dumps(summary, indent=2))
    print(f"Saved figures to {FIG_DIR}")
    print(f"Saved summary to {SUMMARY_DIR / 'round1_summary.json'}")
