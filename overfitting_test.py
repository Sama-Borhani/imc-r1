...
# ═══════════════════════════════════════════════════════════════════════
# EXPERIMENT SETTINGS: Nudged for Sensitivity Testing
# ═══════════════════════════════════════════════════════════════════════
POSITION_LIMIT = 80
IPR = "INTARIAN_PEPPER_ROOT"
ACO = "ASH_COATED_OSMIUM"

# ACO: Nudged from 0.50 to 0.40 to test Jump Protection sensitivity
ACO_EWMA_ALPHA   = 0.05
ACO_BASE_EDGE    = 2.0
ACO_SKEW_FACTOR  = 3.0
ACO_JUMP_IMB     = 0.40  # <--- TEST A: Is 0.50 a "lucky" peak?
ACO_JUMP_SPREAD  = 10.0
ACO_JUMP_BIAS    = 1.0

# IPR: Nudged from 2.0 to 2.5 to test Signal Conviction robustness
IPR_INTERCEPT_ALPHA = 0.08
IPR_PASSIVE_CLIP    = 20
IPR_SIGNAL_THRESH   = 2.5 # <--- TEST B: Does a higher bar kill the alpha?
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # 2. Context & Robust Last Day Detection
        # Detecting Day 1 via IPR price levels to survive traderData resets
        ...
        if IPR in state.order_depths:
            result[IPR] = self._trade_ipr(state.order_depths[IPR], state.position.get(IPR, 0), is_endgame, is_last_day)

        return result, 0, json.dumps(data, separators=(",", ":"))

    def _trade_ipr(self, depth: OrderDepth, pos: int, end: bool, last: bool) -> List[Order]:
        orders = []
        if end and last:
            curr = pos
        ...
        if target > 0:
            # TEST B: Higher score required to justify the 14-tick spread
            if score >= IPR_SIGNAL_THRESH: 
    ...
    def _trade_aco(self, depth: OrderDepth, pos: int, end: bool, last: bool, fv: float) -> List[Order]:
        ...
        # TEST A: More aggressive jump protection
        imb = self._imbalance(bv, av)
        spread = ba - bb
