...
# Debug mode: no MAF while the engine is still being repaired.
MAF_BID = 0

# Real endgame timestamps for this simulator are ~0..99900, not 0..999000.
ENDGAME_START = 96000
ENDGAME_HARD = 98500

# Inventory safety
OSMIUM_EMERGENCY_POSITION = 60
PEPPER_EMERGENCY_POSITION = 60
EMERGENCY_UNWIND_THRESHOLD = 0.5

# Very light OSMIUM one-sided-book protection.
OSMIUM_EXTREME_IMBALANCE = 0.70

# Mild PEPPER short asymmetry only.
PEPPER_SHORT_THRESHOLD_MULT = 1.15
PEPPER_SHORT_SIZE_MULT = 0.80
...
class Trader:
    ...
    def load_memory(self, trader_data: str) -> None:
        ...
        try:
            ...
            self.memory = ProductMemory(
                pepper_intercept_samples=list(raw.get("pepper_intercept_samples", []))[-PEPPER_INTERCEPT_WINDOW:],
    ...
    def trade_osmium(self, state: TradingState, order_depth: OrderDepth) -> List[Order]:
        ...
        # Only keep a very light protection: don't keep offering the vulnerable side
        # on extremely one-sided books when inventory is not already helping.
        if imbalance is not None and imbalance >= OSMIUM_EXTREME_IMBALANCE and position <= 0:
            allow_passive_sell = False
        elif imbalance is not None and imbalance <= -OSMIUM_EXTREME_IMBALANCE and position >= 0:
            allow_passive_buy = False

        # Emergency unwind.
        if position >= OSMIUM_EMERGENCY_POSITION:
            allow_passive_buy = False
            sell_take_threshold = min(sell_take_threshold, EMERGENCY_UNWIND_THRESHOLD)
            sell_size = min(config.position_limit + position, max(sell_size, 18))
        elif position <= -OSMIUM_EMERGENCY_POSITION:
            allow_passive_sell = False
            buy_take_threshold = min(buy_take_threshold, EMERGENCY_UNWIND_THRESHOLD)
            buy_size = min(config.position_limit - position, max(buy_size, 18))

        # Real close-out logic.
        if state.timestamp >= ENDGAME_START:
            aggression = 1 if state.timestamp < ENDGAME_HARD else 2
            if position > 0:
                allow_passive_buy = False
                sell_take_threshold = min(sell_take_threshold, max(0.0, 1.0 - 0.5 * aggression))
                sell_size = min(config.position_limit + position, max(sell_size, 18 + 4 * aggression))
            elif position < 0:
                allow_passive_sell = False
                buy_take_threshold = min(buy_take_threshold, max(0.0, 1.0 - 0.5 * aggression))
                buy_size = min(config.position_limit - position, max(buy_size, 18 + 4 * aggression))
    ...
    def trade_pepper(self, state: TradingState, order_depth: OrderDepth) -> List[Order]:
        ...
        # Baseline strong-signal behavior, with only mild short asymmetry.
        if z_score >= PEPPER_STRONG_Z:
            sell_size = max(2, int(round(min(16, config.default_order_size + 4) * PEPPER_SHORT_SIZE_MULT)))
        ...
        if position >= PEPPER_EMERGENCY_POSITION:
            allow_passive_buy = False
            sell_take_threshold = min(sell_take_threshold, EMERGENCY_UNWIND_THRESHOLD)
            sell_size = min(config.position_limit + position, max(sell_size, 14))
        elif position <= -PEPPER_EMERGENCY_POSITION:
            allow_passive_sell = False
            buy_take_threshold = min(buy_take_threshold, EMERGENCY_UNWIND_THRESHOLD)
            buy_size = min(config.position_limit - position, max(buy_size, 14))
        ...
        if state.timestamp >= ENDGAME_START:
            ...
            if position > 0:
                allow_passive_buy = False
                sell_take_threshold = min(sell_take_threshold, max(0.0, 1.0 - 0.5 * aggression))
                sell_size = min(config.position_limit + position, max(sell_size, 14 + 4 * aggression))
            elif position < 0:
                allow_passive_sell = False
                buy_take_threshold = min(buy_take_threshold, max(0.0, 1.0 - 0.5 * aggression))
                buy_size = min(config.position_limit - position, max(buy_size, 14 + 4 * aggression))
